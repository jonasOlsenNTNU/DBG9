...
def _calculate_baseline_projection(
    ...
    return {
        ...
        "port_cagr": port_cagr * 100,
        "co2_cagr": co2_cagr * 100,
        "gdp_cagr": gdp_cagr * 100,
        "efficiency_cagr": efficiency_cagr * 100,
        "coal_change_rate": coal_change_pp_per_year,
...
def _calculate_adjusted_projection(
        baseline: dict,
        efficiency_acceleration: float,
        coal_transition_speed: float,
    ...
    historical_coal_rate = baseline["coal_change_rate"]
    ...
    coal_reduction_actual = min(abs(coal_reduction_pp), current_coal_share)

    co2_from_coal_transition = 0.0
    if current_coal_share > 0 and coal_reduction_actual != 0:
        coal_contribution = current_coal_share / 100
    ...
    adjusted_co2 = max(adjusted_co2, 0)
...
def create_scenario_visualization(
    ...
    co2_values = np.concatenate([
        historical_co2,
        [latest_co2, baseline["baseline_co2_2030"]],
        [latest_co2, adjusted["adjusted_co2_2030"]],
    ])
    ...
    color_key = {
        "Historical": "#666666",
        "Baseline 2030": "#386cb0",
        "Adjusted 2030": "#f0027f",
    }
    ...
    plot = plot.redim.range(year=(2000, target_year))
...
- Total CO₂ reduction: **{adjusted['co2_reduction_vs_baseline']:.2f} Mt** ({adjusted['co2_reduction_pct']:.1f}%)
  ...
  - From coal energy transition: {adjusted['co2_from_coal_transition']:.2f} Mt
    ...
    return pn.Column(
        pn.pane.Markdown(
            """
### How this 2030 scenario tool works
...
2. Adjust the efficiency and coal phase-out multipliers to represent stronger or weaker policies.  
3. Compare the **Baseline 2030** and **Adjusted 2030** paths in the chart.

**Reading the graph**

- **Black**: Historical emissions  
- **Grey**: Baseline 2030 (continue recent trend)  
- **Blue**: Adjusted 2030 (with your policy assumptions)  

A downward adjusted line relative to the baseline indicates **stronger decoupling** – emissions fall or grow more slowly even if port traffic keeps increasing.
            """,
            sizing_mode="stretch_width",
        ),
...
def create_scenario_tab(df: pd.DataFrame) -> pn.Column:
    header = pn.pane.Markdown(
        """
# Decarbonisation scenarios for maritime economies to 2030

This view lets you test **what-if policies** for maritime economies:
can efficiency gains and faster coal phase-out bend the CO₂ curve while ports keep growing?
        """,
        sizing_mode="stretch_width",
        css_classes=["story-header"],
    )

    body = create_scenario_widget(df)
